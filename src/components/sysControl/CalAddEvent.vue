...
import { createNewCalEvent } from '@/services/oms/bases'
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      if (createRes === '00000') {
        ...
        ElNotification({
          title: t('notice.noticeTitle'),
          message: t('notice.createCalendarNoPerErrorMsg'),
          type: 'error'
        });
      } else {
        ElNotification({
          title: t('notice.noticeTitle'),
          message: t('notice.createCalendarErrorMsg'),
          type: 'error'
        });
      }
    }
...
<template>
  <el-dialog
    v-model="isVisible"
    :title="t('calendar.addEventDialog.title')"
    width="500px"
