...
const permissionTableColumns = computed(() => {
  return [
    {
      prop: 'comPerNo',
      label: t('comPerRoles.defRoles.perCode'),
      width: '120'
    },
    {
      prop: locale.value === 'zh-TW' ? 'comPerName' : 'comPerEngName',
      label: t('comPerRoles.defRoles.perName'),
      width: '200'
    },
    {
      prop: locale.value === 'zh-TW' ? 'comPerDesc' : 'comPerEngDesc',
      label: t('comPerRoles.defRoles.perDesc'),
      width: '300'
    }
  ]
})
    ...
    <el-table-column type="expand">
      <template #default="props">
        <div class="permission-details">
          <h4>{{ getRoleName(props.row) }} {{ t('comPerRoles.defRoles.permissionHint') }}</h4>
    ...
    <el-table-column v-if="canViewUsers" :label="t('comPerRoles.defRoles.users')" :width="usersColumnWidth">
      <template #default="scope">
        <el-button @click.stop="showUsers(scope.row)" type="primary" link>
          {{ t('comPerRoles.defRoles.viewUsers') }}
        </el-button>
      </template>
    </el-table-column>
  </el-table>
